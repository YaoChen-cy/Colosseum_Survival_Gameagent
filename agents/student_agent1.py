...
def compute_dic(chess_board, cur_pos):
    r, c = cur_pos
...
def find_best_step(chess_board, my_pos, adv_pos, max_step):
    ...
    weight = 10000
    q=compute_dic(chess_board, my_pos)
    best_step = (my_pos, q[0])
    for move in next_list:
        if check_valid_step(chess_board, adv_pos, max_step, my_pos, move):
            ...
            for i in available_direction:
                r, c = move
                temp_board[r][c][i] = True
                end_game, winner = check_endgame(temp_board, move, adv_pos)
                if end_game:
                    if winner == 1:
                        return move, i
                    elif winner == -1:
                        available_direction.remove(i)
                temp_board[r][c][i] = False
